...
qboolean VID_LoadRefresh( char *name )
{
...
#else
	{
		...
		char fail[32];
		fail[31] = 0;
		if ((r = DosLoadModule(fail, 31, name, &hreflib_library))) {
			Com_Printf( "LoadLibrary(\"%s\") failed: %d(%s)\n", name , r, fail);
			return false;
		}
	}
#endif
...
#else
	if (DosQueryProcAddr(hreflib_library, 0, "GetRefAPI", (PFN *)&GetRefAPI))
		Com_Error( ERR_FATAL, "DosQueryProcAddr failed on %s", name );
#endif
...
#else
	if (
		DosQueryProcAddr(hreflib_library, 0, "RW_IN_Init", (PFN *)& RW_IN_Init_fp) ||
		DosQueryProcAddr(hreflib_library, 0, "RW_IN_Shutdown", (PFN *)& RW_IN_Shutdown_fp) ||
		DosQueryProcAddr(hreflib_library, 0, "RW_IN_Commands", (PFN *)& RW_IN_Commands_fp) ||
		DosQueryProcAddr(hreflib_library, 0, "RW_IN_Move", (PFN *)& RW_IN_Move_fp) ||
		DosQueryProcAddr(hreflib_library, 0, "RW_IN_Frame", (PFN *)& RW_IN_Frame_fp))
#endif
...
#else
	if (
		DosQueryProcAddr(hreflib_library, 0, "KBD_Init", (PFN *)& KBD_Init_fp) ||
		DosQueryProcAddr(hreflib_library, 0, "KBD_Update", (PFN *)& KBD_Update_fp) ||
		DosQueryProcAddr(hreflib_library, 0, "KBD_Close", (PFN *)& KBD_Close_fp))
#endif
...
#else
		sprintf( name, "r_%s", vid_ref->string );
#endif
